...
const deleteProduct = async (id) => {
  ...
  const deleteProduct = async (productId, token, onSuccess, onError) => {
    if (!window.confirm("Are you sure you want to delete this product?")) return;
  
    try {
      const response = await query({
        method: "DELETE",
        url: `/admin/admin-products/product/delete?product_id=${productId}`,
        token: token,
      });
  
      if (response.success) {
        alert("Product deleted successfully!");
        onSuccess?.(); // Refresh list or navigate
      } else {
        ...
        <div className="modal-dialog modal-dialog-centered modal-lg" role="document">
                    <div className="modal-content">
                      <div className="modal-body">
                        ...
                        <h1>{isEditMode ? "Update Product" : "Add a New Product"}</h1>
                        <p>{isEditMode ? "Manage Product":"Create New Product"} </p>
                        {alertText && <div className="alert alert-info mt-md">{alertText}</div>}
