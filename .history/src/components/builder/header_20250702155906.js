...
import { useSplitFullName } from "../../hooks/useSplitFullName";


const Header = () => {

  const userData = useSelector((state) => state);
  const { fname, mname, lname } = useSplitFullName(userData?.data.data.user.name);
  // const initials = userData?.user?.user?.name
  // .split(" ")           
  // .map(word => word[0])
  // .join("");
  ...
  .join("")}</h3>
          ...
          <h4 className="title-case text-light">
            {" "}
            {userData?.user.user.name} <br />
            <span className="summary__label font-9 role">{userData?.user.user.userType}</span>
          </h4>
        </div>
